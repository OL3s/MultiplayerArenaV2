...
- Player visuals use temporary SVG sprites in `Assets/Players/`: front and back body images plus a right-facing pistol image. The player body sprite flips only on its own X scale, switches to the back sprite only when aiming upward enough, and leaves the root/collision transform unchanged.
