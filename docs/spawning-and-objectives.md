...
- Main/core neutral objective: the map-center objective. There should be exactly one per match map, and it is not used for map-spawned gear.
- Secondary neutral objectives: spread-out candidate objective spots used by game modes that need random or rotating locations. When a secondary neutral objective is not currently active as an objective, loadout modes such as `MapPickups` may use that location for occasional weapon, gear, and gadget pickup spawns.
